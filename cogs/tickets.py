...
async def log_ticket_action(guild, action: str, metadata: dict, extra: str = ""):
    """
    Log ticket actions (creation, claim, priority, close, transcript) to the configured log channel.
    """
    try:
        config = load_data('config') if callable(globals().get('load_data', None)) else {}
        log_channel_id = config.get("ticket_log_channel")
        ...
        embed = discord.Embed(
            ...
            description=f"Ticket ID: {metadata.get('ticket_id')}\nUser: <@{metadata.get('user_id')}>\nCategory: {metadata.get('category')}\nStatus: {metadata.get('status')}\n{extra}",
